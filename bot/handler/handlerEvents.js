...
function getRole(threadData, senderID) {
        const { adminBot, vipUsers, botDevelopers } = global.GoatBot.config;
        const adminBotList = adminBot || [];
        const vipUsersList = vipUsers || [];
        const botDevelopersList = botDevelopers || [];

        if (!senderID)
                return 0;

        if (botDevelopersList.includes(senderID))
                return 4;
        if (adminBotList.includes(senderID))
                return 2;
        if (vipUsersList.includes(senderID))
                return 3;
                        ...
                        if (needRole > role) {
                                if (!hideNotiMessage.needRoleToUseCmd) {
                                        if (needRole == 1)
                                                ...
                                                return await message.reply(`⚠️ Only VIP users can use the command "${commandName}"`);
                                        else if (needRole == 4)
                                                return await message.reply(`⚠️ Only Bot Developers can use the command "${commandName}"`);
                                }
                                else {
                                        return true;
                                }
